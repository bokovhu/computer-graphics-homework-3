...
public: 
	...
	Ellipsoid(vec3 r) : radius (r.x, r.y, r.z) { Create(); }
	...
	VertexData GenVertexData(float u, float v) {
		...
		vd.position = vd.normal = vec3(
			radius.x * cosf(_u) * sinf(_v), 
			radius.y * sinf(_u) * sinf(_v), 
			radius.z * cosf(_v)
		);
...
public:
	...
	void Build() {
		...
		// Geometries
		Ellipsoid * sphere = new Ellipsoid(vec3 (1.0, 0.5, 0.5));

		Geometry* bottle = new KleinBottle(3.0, 4.0, 2.0);
		...
		Object * sphereObject2 = new Object(phongShader, material0, texture15x20, bottle);
		sphereObject2->translation = vec3(0, 0, 0);
